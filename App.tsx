  ...
  const handleEnemyDefeated = (rank: Rank, exp: number, gold: number, storyId?: number) => {
      ...
      if (storyId !== undefined && storyId === player.storyStage) {
          ...
          if (storyId === 1) { 
              ...
              const igris: Companion = { id: 'igris', name: '이그리스', rank: Rank.A, description: '핏빛의 기사단장', type: 'SHADOW', role: '기사단장', attackBonus: 50 };
              updates.companions = [...player.companions, igris];
              addLog("그림자 병사 '이그리스'를 획득했습니다.", 'gain');
          }
          if (storyId === 2) { 
              const iron: Companion = { id: 'iron', name: '아이언', rank: Rank.A, description: '강철의 육체', type: 'SHADOW', role: '탱커', attackBonus: 40 };
              updates.companions = [...(updates.companions || player.companions), iron];
              addLog("그림자 병사 '아이언'을 획득했습니다.", 'gain');
          }
          if (storyId === 4) { 
              const beru: Companion = { id: 'beru', name: '베르', rank: Rank.S, description: '개미의 왕', type: 'SHADOW', role: '군단장', attackBonus: 200 };
    ...
    <div className="min-h-screen w-full bg-system-dark text-system-text flex flex-col p-2 lg:p-6 relative">
      ...
      {isCodeModalOpen && (
           <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/90 backdrop-blur-sm animate-in fade-in duration-200 p-4">
                <div className="bg-system-panel border border-system-blue p-6 rounded-lg w-full max-w-sm shadow-[0_0_30px_rgba(0,168,255,0.2)]">
                    <h2 className="text-system-blue font-bold tracking-widest mb-4 border-b border-system-blue/30 pb-2">ADMINISTRATOR CODE</h2>
                    <input 
                        type="password" 
                        value={codeInput}
                        onChange={(e) => setCodeInput(e.target.value)}
                        placeholder="ACCESS CODE"
                        className="w-full bg-black border border-gray-700 text-center font-mono text-lg text-white p-2 mb-4 focus:border-system-blue outline-none"
                        autoFocus
                    />
                    <div className="flex gap-2">
                        <button 
                            onClick={handleSubmitCode}
                            className="flex-1 bg-system-blue text-black font-bold py-2 rounded hover:bg-blue-600 transition-colors"
                        >
                            SUBMIT
                        </button>
                        <button 
